...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		/**
		 * 这里通过name获取beanName。这里不使用name直接作为beanName有两个原因
		 * 	1 name可能会以 & 字符开头，表明调用者想获取FactoryBean 本身，而非FactoryBean实现类所创建的bean。在beanFactroy中
		 * 	  FactoryBean的实现类和其他的bean存储方式是一致的。即<beanName, bean>,beanName中是没有&这个字符的。所以我们需要将
		 * 	  name的首字符&移除，这样才能从缓存里取到FactoryBean实例
		 * 	2 还有别名的问题，转换需要
		 */
